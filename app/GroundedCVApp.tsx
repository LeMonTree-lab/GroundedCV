...
function softenClaimText(text: string, sourceFacts: Fact[]) {
  ...
  // A selected “weaken” decision must visibly change the sentence. This
  // fall-back deliberately narrows the assertion to source evidence only.
...
export default function GroundedCVApp() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const saved = normalizeGroundedProject(JSON.parse(raw) as GroundedProject);
        window.queueMicrotask(() => setSavedProject(saved));
      }
    } catch {
  ...
  useEffect(() => {
    ...
    window.queueMicrotask(() => setSavedProject(project));
  }, [hydrated, project, screen]);

  const factAssets = getFactAssets(project);
  const experiences = factAssets.filter(isExperienceAsset);
  const assets = factAssets.filter((record): record is FactAsset => !isExperienceAsset(record));
  ...
  function startProject(nextProject: GroundedProject) {
    const normalized = normalizeGroundedProject(nextProject);
    setProject(normalized);
    setSelectedExperience(getFactAssets(normalized).find(isExperienceAsset)?.id ?? "");
    setActiveStep(0);
    setScreen("workspace");
  }
  ...
  function updateFact(factId: string, status: FactStatus) {
    setProject((current) => ({
      ...current,
      resume: undefined,
      factAssets: getFactAssets(current).map((record) => ({
        ...record,
        facts: record.facts.map((fact) => fact.id === factId ? { ...fact, status } : fact),
      })),
    }));
  ...
  function upsertExperience(experience: Experience) {
    setProject((current) => {
      const records = getFactAssets(current);
      const exists = records.some((item) => item.id === experience.id);
      return {
        ...current,
        resume: undefined,
        factAssets: exists ? records.map((item) => item.id === experience.id ? experience : item) : [...records, experience],
      };
    });
  ...
  function deleteExperience(experienceId: string) {
    ...
    setProject((current) => ({
      ...current,
      resume: undefined,
      factAssets: getFactAssets(current).filter((item) => item.id !== experienceId),
    }));
  ...
  function upsertAsset(asset: FactAsset) {
    setProject((current) => {
      const records = getFactAssets(current);
      const exists = records.some((item) => item.id === asset.id);
      return { ...current, resume: undefined, factAssets: exists ? records.map((item) => item.id === asset.id ? asset : item) : [...records, asset] };
    });
    setNotice("事实资产已保存");
    window.setTimeout(() => setNotice(""), 1800);
  }
  ...
  function deleteAsset(assetId: string) {
    setProject((current) => ({
      ...
      factAssets: getFactAssets(current).filter((item) => item.id !== assetId),
    }));
    setNotice("事实资产已删除");
    window.setTimeout(() => setNotice(""), 1800);
  }
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="top-actions">
            <button type="button" className={aiSettings.apiKey ? "api-ready" : "ghost-button"} onClick={() => setApiSettingsOpen(true)}>{aiSettings.apiKey ? "自带 Key 已连接" : "开放测试 / AI 设置"}</button>
        ...
        {activeStep === 0 ? (
          ...
          <RiskCenter risks={liveRisks} claimCount={project.resume?.claims.length ?? 0} onSelectRisk={setSelectedRiskId} onNext={() => setActiveStep(4)} />
        ) : activeStep === 4 ? (
          <InterviewTest
...
function ResumeStudio({
  ...
  const sourceFacts = getFactAssets(project).flatMap((record) => record.facts);
...
function RiskCenter({ risks, claimCount, onSelectRisk, onNext }: { risks: LiveRisk[]; claimCount: number; onSelectRisk: (riskId: string) => void; onNext: () => void }) {
  const [selected, setSelected] = useState("");
  const risk = risks.find((item) => item.id === selected) ?? risks[0];
  const counts = { high: risks.filter((item) => item.severity === "high").length, medium: risks.filter((item) => item.severity === "medium").length };
  if (!claimCount) return <EmptyStage title="先生成当前项目的简历" copy="Claim 检测会读取你刚刚生成的简历，而不是展示固定示例。" />;
  if (!risk) return <div className="page"><div className="page-heading"><div><span className="eyebrow">STEP 04 · CLAIM INSPECTOR</span><h1>所有 Claim 均在事实边界内</h1><p>已检查 {claimCount} 条当前 Claim；没有发现新增数字、责任/结果升级、上线状态错配或缺少来源的表述，因此无需进入追问。</p></div><button className="primary-button" type="button" onClick={onNext}>查看最终简历 <span>→</span></button></div><div className="missing-callout"><span>✓</span><div><strong>自动通过，不等于遗漏检查</strong><p>技能清单、教育信息、奖项与来源完整的普通事实会直接保留；只有需要用户补证或决定弱化的 Claim 才会被列为风险。</p></div></div></div>;
  return (
    <div className="page">
      <div className="page-heading">
...
function InterviewTest({ risks, risk: target, response, completedClaimIds, onSave, onSelectRisk, onNext }: { risks: LiveRisk[]; risk?: LiveRisk; response?: InterviewResponse; completedClaimIds: string[]; onSave: (response: InterviewResponse) => void; onSelectRisk: (riskId: string) => void; onNext: () => void }) {
  ...
  if (!target) return <div className="page"><div className="page-heading"><div><span className="eyebrow">STEP 05 · INTERVIEW PRESSURE TEST</span><h1>没有需要追问的 Claim</h1><p>风险检测没有发现需要补证或弱化的表述，本轮无需进行面试压力测试。</p></div><button className="primary-button" type="button" onClick={onNext}>跳过追问 <span>→</span></button></div></div>;
    ...
    <div className="page">
      <div className="page-heading">
        <div>
          ...
          <h1>统一管理真实事实，再让 AI 写简历</h1>
          <p>经历、技能、教育研究、奖项证书和作品链接都在同一事实资产库中管理。来源清楚的描述会自动收录；只需核查数字、职责边界、能力等级和结果等高风险内容。</p>
        </div>
        <button className="primary-button" type="button" onClick={onNext}>确认并分析岗位 <span>→</span></button>
      </div>

      <div className="metric-strip">
        <div><span className="metric-value">{experiences.length + assets.length}</span><span>张事实资产卡</span></div>
...
function AssetLibrary({
  ...
  return <section className="asset-library">
    <div className="asset-library-head">
      <div><span className="eyebrow">FACT ASSET LIBRARY</span><h2>支持型事实资产</h2><p>技能、教育与研究、奖项证书、作品与链接与经历卡共享同一事实库，会参与 JD 匹配，并分别进入简历的对应栏目，不会被混入“其他经历”。</p></div>
...
function AiSettingsDialog({
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section className="experience-modal api-settings-modal" role="dialog" aria-modal="true" aria-labelledby="ai-settings-title">
      ...
      <div className="api-privacy-note"><strong>当前为开放测试模式，也可以使用自己的 Key。</strong><p>不填写 Key 时，系统使用服务器端配置的匿名测试额度，暂不限制浏览器次数。填写自己的 DeepSeek Key 时仅保留在当前页面，直接请求 DeepSeek；网站不接收、不保存你的 Key。简历和岗位内容默认只保存在当前浏览器。</p></div>
      <div className="modal-grid">
      ...
      {error && <p className="modal-error" role="alert">{error}</p>}
      <div className="modal-actions"><button type="button" className="ghost-button" onClick={() => { onSave({ apiKey: "", model }); onClose(); }}>使用开放测试</button><button type="button" className="primary-button" onClick={save}>保存到当前会话</button></div>
    </section>
  </div>;
}
